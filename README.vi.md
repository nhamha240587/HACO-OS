...
- 🏠 **Local LLM (self-hosted)** — định tuyến model `ollama/…`, `vllm/…`, `local/…` tới engine nội bộ. Source/prompt **không rời hạ tầng**, chi phí API = 0, nhưng vẫn đi qua đủ lớp quota + audit + ROI.
...
| **Local LLM** | Ollama / vLLM / mọi endpoint OpenAI-compatible; định tuyến qua tiền tố `ollama/`·`vllm/`·`local/`; chi phí $0 nhưng vẫn đủ governance |
